"""
RABIT-KV Final — frozen target-bit-aware KV-cache quantization for Llama-3.1-8B.

THIS FILE IS THE FINAL RESEARCH SCRIPT.

Default behavior:
    modal run rabit_kv_final.py --mode final --bit-modes "16,8,4,3,2"

In default final mode, the script does NOT search through old V13/V14/V15/V16/V17
method names. It evaluates the frozen RABIT-KV policy selected from the ablation
study:

    8-bit target  -> SYM K32V64 R0
    4-bit target  -> SYM G64 R0
    3-bit target  -> SYM G32 R4
    2-bit target  -> K3V2 Kseq32 Vaff32 R4

This is ONE method with target-bit-specific operating points.
The earlier V13-V18 scripts were ablations used to discover this final policy;
they are not reported as separate final methods.

Optional ablation mode:
    modal run rabit_kv_final.py --mode final --show-candidates

With --show-candidates, the script evaluates the wider candidate region for each
bit budget. That is for debugging/ablation only, not the main final table.

What this benchmark measures:
    - Perplexity impact from quantized KV cache
    - Logical compressed KV storage, including packed codes and metadata
    - Script token/s, which is NOT production serving speed

Important limitation:
    This is still a fake-quant attention benchmark: compressed KV is dequantized
    before the next forward pass. Real latency/memory deployment would require
    packed-KV attention kernels or vLLM integration.
"""

import modal

app = modal.App("rabit-kv-2bit-component-ablation")
...
@app.function(gpu="H100", image=image, timeout=21600, volumes={"/model_cache": model_cache})
def run_benchmark(model_name: str = "LLM-Research/Meta-Llama-3.1-8B-Instruct", mode: str = "final", max_ppl_increase_pct: float = 5.0, residual_tokens: int = 16, bit_modes: str = "16,2", show_candidates: bool = True, profile_heads: int = 0, profile_4bit_heads: int = 0, profile_layers: int = 0, two_bit_quality_cap_pct: float = 5.0):
    import gc
    import math
    import statistics
    import time
    ...
    profile_heads = int(profile_heads)
    profile_4bit_heads = int(profile_4bit_heads)
    two_bit_quality_cap_pct = float(two_bit_quality_cap_pct)
    profile_layers = int(profile_layers)
    ...
    if max_ppl_increase_pct < 0:
        raise ValueError("max_ppl_increase_pct must be non-negative")
    if residual_tokens < 0 or residual_tokens >= MAX_TOKENS:
        raise ValueError("residual_tokens must be >=0 and smaller than MAX_TOKENS")
    if profile_heads < 0:
        raise ValueError("profile_heads must be non-negative")
    if profile_4bit_heads < 0:
        raise ValueError("profile_4bit_heads must be non-negative")
    if two_bit_quality_cap_pct < 0:
        raise ValueError("two_bit_quality_cap_pct must be non-negative")
    if profile_layers < 0:
        raise ValueError("profile_layers must be non-negative")
    profile_4bit_heads = min(profile_4bit_heads, profile_heads)
    if not BIT_MODES:
    ...
    MODE_CFG = {
        "quick": {"num_windows": 2, "profile_windows": 2},
        "final": {"num_windows": 4, "profile_windows": 2},
        "paper": {"num_windows": 8, "profile_windows": 4},
    }
    NUM_WINDOWS = MODE_CFG[mode]["num_windows"]
    PROFILE_WINDOWS = MODE_CFG[mode]["profile_windows"]
    TARGET_BITS = [2]
    ...
    print(f"Independent windows: {NUM_WINDOWS}")
    print(f"Profile windows: {PROFILE_WINDOWS}")
    print(f"Bit modes: {BIT_MODES}")
    if show_candidates:
        print(f"Ablation profiling: top {profile_heads} heads within top {profile_layers} layers; top {profile_4bit_heads} heads at 4-bit, rest at 3-bit")
    else:
        print("Final policy: 8b=SYM K32V64 R0, 4b=SYM G64 R0, 3b=SYM G32 R4, 2b=K3V2 Kseq32 Vaff32 R4")
    print("Method: RABIT-KV Final — one frozen target-bit-aware KV-cache quantizer.")
    if show_candidates:
        print("Ablation mode: evaluates the wider RABIT-KV candidate space for each bit budget.")
    else:
        print("Final mode: evaluates ONLY the frozen RABIT-KV policy selected for the paper table.")
    print("Memory = logical KV storage only. Script speed is not optimized production latency.")
    print()
    ...
    def quantize_tensor_group(tensor, bits, method, side):
        ...
        if bool(method.get("symmetric_absmax", False)):
            ...
            q = {
                ...
                "pad": pad,
                "absmax": q_abs.to(MODEL_DTYPE).contiguous(),
                "group_size": group_size,
                "clip_pct": clip_pct,
                "per_token_scale": bool(method.get("per_token_scale", False)),
            }
            if token_absmax is not None:
                q["token_absmax"] = token_absmax.to(MODEL_DTYPE).contiguous()
            return q
        ...
        q = {
            ...
            "orig_shape": orig_shape,
            "pad": pad,
            "min": q_min.to(MODEL_DTYPE).contiguous(),
    ...
    def quantize_tensor_group_codebook(tensor, bits, method, side):
        ...
        return {
            ...
            "orig_shape": orig_shape,
            "pad": int(pad),
            "codebook": centers.to(MODEL_DTYPE).contiguous(),
    ...
    def dequantize_tensor(q):
        ...
        if q.get("style") in {"group_codebook", "seq_group_codebook"}:
            ...
            levels = 2 ** int(q["bits"])
            x = ((codes / (levels - 1)) * 2.0 - 1.0) * q["absmax"].float()
        else:
            x = codes * q["scale"].float() + q["min"].float()
    ...
    def stored_tensor_bytes(q):
        ...
        if "codebook" in q:
            total = tensor_bytes(q["packed"]) + tensor_bytes(q["codebook"])
        elif "absmax" in q:
            total = tensor_bytes(q["packed"]) + tensor_bytes(q["absmax"])
        else:
            total = tensor_bytes(q["packed"]) + tensor_bytes(q["min"]) + tensor_bytes(q["scale"])
        if "token_absmax" in q:
            total += tensor_bytes(q["token_absmax"])
        return total
    ...
    def quantize_cache(cache, target_bits, method):
        ...
        for layer_idx, (k, v) in enumerate(kv_tuple):
            ...
            k_bits = int(method.get("k_target_bits", layer_bits))
            v_bits = int(method.get("v_target_bits", layer_bits))
            k_bits = max(k_bits, layer_bits) if method.get("k_target_bits_is_floor", False) else k_bits
            v_bits = max(v_bits, layer_bits) if method.get("v_target_bits_is_floor", False) else v_bits
            output.append(
                (
    ...
    # -------------------------------------------------------------
    def final_configs_for_bit(bits):
        """Frozen final RABIT-KV policy used for the paper table.

        These four configurations are the final quantizer. The wider candidate
        space below is only for ablation/debugging when --show-candidates is set.
        """
        if bits == 16:
            return [("fp16 baseline", method_by_id["p3_uniform"])]
        if bits == 8:
            return [("SYM K32V64 R0", rabit_config(
                8, k_bits=8, v_bits=8,
                k_axis="channel", v_axis="token",
                k_group=32, v_group=64,
                residual=0, symmetric=True,
                label="SYM K32V64 R0",
            ))]
        if bits == 4:
            return [("SYM G64 R0", rabit_config(
                4, k_bits=4, v_bits=4,
                k_axis="channel", v_axis="token",
                k_group=64, v_group=64,
                residual=0, symmetric=True,
                label="SYM G64 R0",
            ))]
        if bits == 3:
            return [("SYM G32 R4", rabit_config(
                3, k_bits=3, v_bits=3,
                k_axis="channel", v_axis="token",
                k_group=32, v_group=32,
                residual=4, symmetric=True,
                label="SYM G32 R4",
            ))]
        if bits == 2:
            return [("K3V2 Kseq32 Vaff32 R4", rabit_config(
                2, k_bits=3, v_bits=2,
                k_axis="seq_affine", v_axis="token_affine",
                k_group=32, v_group=32,
                residual=4, symmetric=False,
                label="K3V2 Kseq32 Vaff32 R4",
            ))]
        raise ValueError(f"No final RABIT-KV policy is defined for {bits}-bit.")

    def candidates_for_bit(bits):
        """Focused 2-bit component ablation.

        Each row changes one major component while keeping the evaluation
        dataset, model, windowing, and memory accounting identical.
        """
        if bits == 16:
            return [("fp16 baseline", method_by_id["p3_uniform"])]
        if bits != 2:
            raise ValueError(
                "This focused ablation supports only bit_modes='16,2'."
            )
        ...
        return [
            ...
            (
                ...
                make_v16_kivi_affine(
                    ...
                    v_affine=True,
                    pruned=False,
                    label="2-bit affine-axis ablation",
                ),
            ),
            (
                "D K2V3 Affine R4",
            ...
            (
                ...
                make_v16_kivi_affine(
                    ...
                    v_affine=True,
                    pruned=False,
                    label="remove final residual",
                ),
            ),
            (
                "F FINAL K3V2 Affine R4",
                make_v18_k3v2_meta(
                    k_axis="seq_affine",
                    v_affine=True,
                    k_seq_group=32,
                    ...
                    v_outlier_tokens=0,
                    pruned=False,
                    label="final RABIT-KV 2-bit policy",
                ),
            ),
        ]
    ...
    print()
    fp16_method = method_by_id["p3_uniform"]
    fp16_trials = [run_one_trial(16, w, fp16_method) for w in windows]
    fp16_kv_kb = statistics.median(r["pure_kv_kb"] for r in fp16_trials)
    fp16_loss = statistics.median(r["loss"] for r in fp16_trials)
    fp16_ppl = math.exp(min(fp16_loss, 50))
    max_allowed_ppl = fp16_ppl * (1.0 + max_ppl_increase_pct / 100.0)

    fp16_row = {
        "bits": 16,
        "candidate": "fp16 baseline",
        "method": "fp16 baseline",
        "loss": fp16_loss,
        "loss_stdev": statistics.stdev([r["loss"] for r in fp16_trials]) if len(fp16_trials) > 1 else 0.0,
        "ppl": fp16_ppl,
        "ppl_delta": 0.0,
        "pure_kv_kb": fp16_kv_kb,
        "pure_kv_mb": fp16_kv_kb / 1024,
        "compression": 1.0,
        "ms": statistics.median(r["ms"] for r in fp16_trials),
        "pass_budget": True,
        "note": "uncompressed KV",
    }

    # Profile Llama-3.1 only for ablation/search runs. The frozen final
    # RABIT-KV policy below does not use profiled-head protection, so profiling
    # would only waste time in the main paper run.
    if show_candidates and profile_heads > 0:
        print("Profiling Llama-3.1 layer/head sensitivity map for ablation candidates...")
        profile_input_windows = windows[:PROFILE_WINDOWS]
        pruned_head_map, profiled_head_rank = profile_llama31_head_map(
            profile_input_windows,
            target_bits=2,
            top_heads=profile_heads,
            top_4bit_heads=profile_4bit_heads,
            max_layers=profile_layers,
        )
    else:
        pruned_head_map, profiled_head_rank = {}, []

    # =============================================================
    # TurboQuant-style Pareto-knee objective
    # =============================================================
    #
    # v13 selected the lowest-memory Pareto row inside a wide quality band.
    # That was too aggressive for 2-bit: it improved compression but let PPL explode.
    #
    # v13 keeps RABIT-KV as the same method, but changes selection to a Pareto-knee score:
    #   - higher compression is good
    #   - lower PPL increase is good
    #   - there is no fixed pass/fail PPL threshold
    #
    # Score:
    #   tradeoff_score = compression / (1 + max(0, PPL_delta_pct) / scale)
    #
    # The scale is larger for lower bits, allowing more quality loss at lower memory,
    # but not blindly selecting rows where PPL has exploded.
    QUALITY_SCALE_BY_BIT = {
        8: 20.0,
        4: 18.0,
        3: 24.0,
        2: 28.0,
    ...
    def classify_tier(bits, ppl_delta_pct):
        if bits == 16:
            return "full-cache"
        if ppl_delta_pct <= 5.0:
            return "near-lossless"
        if ppl_delta_pct <= 12.0:
            return "balanced"
        if ppl_delta_pct <= 25.0:
            return "aggressive"
        return "quality-drop"

    def add_tradeoff_fields(row, bits):
        row = dict(row)
        row["ppl_delta_pct"] = 100.0 * (row["ppl"] / fp16_ppl - 1.0)

        scale = QUALITY_SCALE_BY_BIT.get(bits, 16.0)
        quality_penalty = 1.0 + max(0.0, row["ppl_delta_pct"]) / scale

        row["quality_scale"] = scale
        row["tradeoff_score"] = row["compression"] / quality_penalty
        row["effective_kv_bits"] = 16.0 / row["compression"] if row["compression"] > 0 else float("inf")
        row["tier"] = classify_tier(bits, row["ppl_delta_pct"])
        return row

    fp16_row = add_tradeoff_fields(fp16_row, 16)
    fp16_row["tradeoff_score"] = 1.0

    all_candidate_rows = [fp16_row]
    selected_rows = [fp16_row]
    pareto_rows_by_bit = {}
    top_pareto_by_bit = {}

    for bits in [b for b in BIT_MODES if b != 16]:

        print("=" * 120)
        if show_candidates:
            print(f"Target bit {bits}: RABIT-KV ablation configuration search")
        else:
            print(f"Target bit {bits}: evaluating frozen RABIT-KV final configuration")
        print("=" * 120)

        bit_rows = []
        for cand_name, method in candidates_for_bit(bits):
            print(f"  RABIT-KV[{bits}b] config: {cand_name}...")
            trials = [run_one_trial(bits, w, method) for w in windows]
            row = summarize_trials(bits, method, trials, fp16_kv_kb, fp16_loss)
            row["bits"] = bits
            row["candidate"] = cand_name
            row["config"] = cand_name
            row["method"] = "RABIT-KV"
            row["pass_budget"] = row["ppl"] <= max_allowed_ppl  # legacy only; not the v13 objective
            row["note"] = method.get("note", "")
            row = add_tradeoff_fields(row, bits)
            print(
                f"  -> PPL {row['ppl']:.2f}, PPLΔ {row['ppl_delta_pct']:.1f}%, "
                f"KV {row['pure_kv_mb']:.3f} MB, Comp {row['compression']:.1f}x, "
                f"Score {row['tradeoff_score']:.3f}"
            )
            bit_rows.append(row)
            all_candidate_rows.append(row)

        # Pareto frontier: no other row is both better/equal PPL and better/equal memory.
        pareto = []
        for r in bit_rows:
            dominated = False
            for q in bit_rows:
                ...
                if (
                    q["ppl"] <= r["ppl"]
                    and q["pure_kv_mb"] <= r["pure_kv_mb"]
                    and (q["ppl"] < r["ppl"] or q["pure_kv_mb"] < r["pure_kv_mb"])
                ):
                    dominated = True
                    break
            if not dominated:
        ...
        pareto_sorted_by_memory = sorted(pareto, key=lambda r: (r["pure_kv_mb"], r["ppl"]))
        pareto_sorted_by_score = sorted(pareto, key=lambda r: (-r["tradeoff_score"], r["pure_kv_mb"], r["ppl"]))
        pareto_rows_by_bit[bits] = pareto_sorted_by_memory
        top_pareto_by_bit[bits] = pareto_sorted_by_score[:8]

        # Selection:
        # For 2-bit/K3V2, the goal is no longer just the highest score.
        # We first ask whether there is a genuinely useful low-memory point:
        # among candidates under a small PPL cap, choose the lowest KV memory.
        if bits == 2:
            quality_ok = [r for r in pareto if r["ppl_delta_pct"] <= two_bit_quality_cap_pct]
            if quality_ok:
                selected = dict(sorted(quality_ok, key=lambda r: (r["pure_kv_mb"], r["ppl"]))[0])
                selected["note"] = f"lowest KV under {two_bit_quality_cap_pct:.1f}% PPL increase"
            else:
                selected = dict(pareto_sorted_by_score[0])
                selected["note"] = "best Pareto-knee score; no candidate met 2-bit/K3V2 quality cap"
        else:
            selected = dict(pareto_sorted_by_score[0])
            selected["note"] = "best Pareto-knee quality-memory score"
        selected["method"] = "RABIT-KV"
        selected["config"] = selected.get("candidate", "")
        print(
            f"Selected {bits}-bit: {selected.get('candidate', '')} | "
            f"PPL {selected['ppl']:.2f}, PPLΔ {selected['ppl_delta_pct']:.1f}%, "
            f"KV {selected['pure_kv_mb']:.3f} MB, Comp {selected['compression']:.1f}x"
        )
        print()

        selected_rows.append(selected)

    selected_rows.sort(key=lambda r: (r["bits"] != 16, -r["bits"] if r["bits"] != 16 else 0))
    all_candidate_rows.sort(key=lambda r: (r["bits"] != 16, -r["bits"] if r["bits"] != 16 else 0, r["pure_kv_mb"]))

    # Cross-bit dominance check: a lower-bit row is not useful if another selected row
    # has both lower/equal PPL and lower/equal KV memory. This catches pseudo-2-bit rows
    # that are effectively worse than 3-bit.
    for r in selected_rows:
        r["dominated_by"] = None
        if int(r["bits"]) == 16:
            continue
        for q in selected_rows:
            if q is r or int(q["bits"]) == 16:
                continue
            if (
                q["ppl"] <= r["ppl"]
                and q["pure_kv_mb"] <= r["pure_kv_mb"]
                ...
                r["dominated_by"] = int(q["bits"])
                break

    quantized_selected = [r for r in selected_rows if r["bits"] != 16]
    best_compression = sorted(quantized_selected, key=lambda r: (r["pure_kv_mb"], r["ppl"]))[0] if quantized_selected else None

    def print_selected_table(rows, width=140):
        print()
        print("TABLE — RABIT-KV SELECTED CONFIGURATION PER TARGET BIT")
        print("-" * width)
        print(
            f"{'Bits':<6}"
            f"{'Selected candidate':<22}"
            f"{'PPL':<10}"
            f"{'PPLΔ%':<9}"
            f"{'PureKV MB':<11}"
            f"{'Comp':<7}"
        ...
        print("-" * width)
        for r in rows:
            candidate = r.get("candidate", r.get("method", ""))
            print(
                f"{r['bits']:<6}"
                f"{candidate:<22}"
                f"{r['ppl']:<10.2f}"
                f"{r['ppl_delta_pct']:<9.1f}"
                f"{r['pure_kv_mb']:<11.3f}"
                f"{r['compression']:<7.1f}"
                f"{r['effective_kv_bits']:<9.2f}"
                f"{r['tradeoff_score']:<9.3f}"
                f"{r['tier']:<15}"
                f"{r['note']}"
            )

    def print_top_pareto_tables(top_by_bit, width=150):
        print()
        print("TOP PARETO-KNEE CONFIGURATIONS PER TARGET BIT")
        print("-" * width)
        print(
            f"{'Bits':<6}{'Candidate':<22}{'PPL':<10}{'PPLΔ%':<9}"
            f"{'PureKV MB':<11}{'Comp':<7}{'EffBits':<9}{'Score':<9}{'Tier':<15}Note"
        )
        print("-" * width)
        for bits in sorted(top_by_bit.keys(), reverse=True):
            for r in top_by_bit[bits]:
                print(
                    f"{r['bits']:<6}{r.get('candidate',''):<22}{r['ppl']:<10.2f}"
                    f"{r['ppl_delta_pct']:<9.1f}{r['pure_kv_mb']:<11.3f}{r['compression']:<7.1f}"
                    f"{r['effective_kv_bits']:<9.2f}{r['tradeoff_score']:<9.3f}{r['tier']:<15}{r['note']}"
                )

    def print_candidate_table(rows, width=170):
        print()
        print("TABLE — ALL RABIT-KV CONFIGURATIONS EVALUATED")
        print("-" * width)
        print(
            f"{'Bits':<6}{'Candidate':<22}{'PPL':<10}{'PPLΔ%':<9}"
            f"{'PureKV MB':<11}{'Comp':<7}{'EffBits':<9}{'Score':<9}{'Tier':<15}Note"
    ...
    # Remote side returns data only; local entrypoint prints one clean final table.
    ...
    return {
        ...
        "pareto_rows_by_bit": pareto_rows_by_bit,
        "top_pareto_by_bit": top_pareto_by_bit,
        "best_compression_selected": best_compression,
        "quality_scale_by_bit": QUALITY_SCALE_BY_BIT,
        "profile_heads": profile_heads,
        "profile_4bit_heads": profile_4bit_heads,
        "profile_layers": profile_layers,
        "profiled_head_map": pruned_head_map,
        "profiled_head_rank": profiled_head_rank[:50],
        "show_candidates": show_candidates,
    }

def print_results_locally(result):
    """Print the focused 2-bit component ablation table."""
    rows = result["candidate_rows"]
    fp16 = next(r for r in rows if int(r["bits"]) == 16)
    two_bit_rows = [
        r for r in rows
        if int(r["bits"]) == 2
    ]
    ...
    order = {
        ...
        "D K2V3 Affine R4": 3,
        "E K3V2 Affine R0": 4,
        "F FINAL K3V2 Affine R4": 5,
    }
    two_bit_rows.sort(
        key=lambda r: order.get(r.get("candidate", ""), 999)
    )

    print()
    print("RABIT-KV 2-BIT COMPONENT ABLATION — Llama 3.1 8B")
    print("=" * 112)
    print(
        f"{'Configuration':<30}"
        f"{'PPL↓':<11}"
    ...
    print(
        f"{'FP16 baseline':<30}"
        f"{fp16['ppl']:<11.2f}"
        f"{fp16['ppl_delta_pct']:<11.1f}"
        f"{fp16['pure_kv_mb']:<11.3f}"
        f"{fp16['compression']:<10.2f}"
        f"{fp16['effective_kv_bits']:<11.2f}"
        f"Uncompressed reference"
    )

    descriptions = {
        "A Uniform2 G32 R0": "ordinary uniform symmetric 2-bit",
        "B Uniform2 G32 R4": "+ four-token FP16 residual",
        "C K2V2 Affine R4": "+ sequence/token affine axes",
        "D K2V3 Affine R4": "protect V instead of K",
        "E K3V2 Affine R0": "mixed K3/V2, remove residual",
        "F FINAL K3V2 Affine R4": "final: K3/V2 + affine + R4",
    }

    by_name = {}
    for row in two_bit_rows:
        name = row.get("candidate", "")
        by_name[name] = row
        print(
            f"{name:<30}"
            f"{row['ppl']:<11.2f}"
            f"{row['ppl_delta_pct']:<11.1f}"
            f"{row['pure_kv_mb']:<11.3f}"
            f"{row['compression']:<10.2f}"
            f"{row['effective_kv_bits']:<11.2f}"
            f"{descriptions.get(name, '')}"
        )

    print("-" * 112)

    def compare(left_name, right_name, label):
        left = by_name[left_name]
        right = by_name[right_name]
        ppl_change = right["ppl"] - left["ppl"]
        kv_change = right["pure_kv_mb"] - left["pure_kv_mb"]
        print(
            f"{label}: PPL change {ppl_change:+.2f}; "
            f"KV change {kv_change:+.3f} MB."
        )
    ...
    compare(
        "A Uniform2 G32 R0",
        "B Uniform2 G32 R4",
        "Residual contribution on uniform 2-bit",
    )
    compare(
        "B Uniform2 G32 R4",
        "C K2V2 Affine R4",
        "Affine-axis contribution at K2/V2",
    )
    compare(
        "C K2V2 Affine R4",
        "F FINAL K3V2 Affine R4",
        "Protecting K with 3 bits",
    )
    compare(
        "E K3V2 Affine R0",
        "F FINAL K3V2 Affine R4",
        "Residual contribution on final K3/V2",
    )
    compare(
        "D K2V3 Affine R4",
        "F FINAL K3V2 Affine R4",
        "K3/V2 versus reversed K2/V3 allocation",
    )

    uniform = by_name["A Uniform2 G32 R0"]
    final = by_name["F FINAL K3V2 Affine R4"]
    ppl_reduction_pct = 100.0 * (
        1.0 - final["ppl"] / uniform["ppl"]
    )
    print()
    print(
        "FINAL VS ORDINARY UNIFORM 2-BIT: "
        f"PPL reduced by {ppl_reduction_pct:.1f}%, "
        f"from {uniform['ppl']:.2f} to {final['ppl']:.2f}; "
        f"KV changes from {uniform['pure_kv_mb']:.3f} MB "
        f"to {final['pure_kv_mb']:.3f} MB."
    )
...
def main(
    ...
    profile_heads: int = 0,
    profile_4bit_heads: int = 0,
    profile_layers: int = 0,
    two_bit_quality_cap_pct: float = 5.0,
):
    result = run_benchmark.remote(
        ...
        profile_4bit_heads=profile_4bit_heads,
        profile_layers=profile_layers,
        two_bit_quality_cap_pct=two_bit_quality_cap_pct,
    )
    print_results_locally(result)