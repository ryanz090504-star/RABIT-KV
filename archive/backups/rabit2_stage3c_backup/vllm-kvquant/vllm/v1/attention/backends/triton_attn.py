...
class TritonAttentionImpl(AttentionImpl):
    ...
    def __init__(
        ...
        self._rabit2_runtime: Rabit2SingleSequenceRuntime | None = None
    ...
    def _forward_rabit_kv2(
        ...
        """Stage-3B2 single-sequence RABIT-KV2 runtime path.

        Initial prefill uses the normal dense Triton context kernel while the
        exact sidecar/cache state is built. Decode then reads immutable closed
        pages directly from the opaque cache with the RABIT packed Triton
        kernel and combines them with the bounded exact open/R4 tail.
        """
        if self.attn_type != AttentionType.DECODER:
            raise NotImplementedError("Stage-3B2 RABIT_KV2 supports decoder attention only")
        if attn_metadata.query_start_loc.shape[0] != 2:
            raise NotImplementedError(
                "Stage-3B2 RABIT_KV2 currently supports one active sequence per batch"
            )
        if self.alibi_slopes is not None or self.sinks is not None or self.logits_soft_cap:
            raise NotImplementedError(
                "Stage-3B2 RABIT_KV2 does not yet support ALiBi, sinks, or softcap"
            )
        if self.sliding_window != (-1, -1) or self.chunk_lookback >= 0:
            raise NotImplementedError(
                "Stage-3B2 RABIT_KV2 does not yet support sliding/chunked attention"
            )
        if attn_metadata.mm_prefix_range_tensor is not None or attn_metadata.rswa_window is not None:
            raise NotImplementedError(
                "Stage-3B2 RABIT_KV2 does not yet support multimodal prefix or R-SWA"
            )
        ...
        k = key[:n]
        v = value[:n]
        block_table_row = attn_metadata.block_table[0]

        # A q_len>1 call is accepted only as an unchunked initial prefill.
        # The one CPU scalar read happens only on prefill, never token-by-token decode.
        if attn_metadata.max_query_len > 1:
            seq_len = int(attn_metadata.seq_lens[0].item())
            if seq_len != n:
                raise NotImplementedError(
                    "Stage-3B2 RABIT_KV2 chunked prefill is not integrated yet"
                )
            self._rabit2_runtime = Rabit2SingleSequenceRuntime(
                self.num_kv_heads, self.head_size, self.head_size
            )
        elif self._rabit2_runtime is None:
            self._rabit2_runtime = Rabit2SingleSequenceRuntime(
                self.num_kv_heads, self.head_size, self.head_size
            )

        assert self._rabit2_runtime is not None
        self._rabit2_runtime.append(k, v, kv_cache, block_table_row)

        if attn_metadata.max_query_len > 1:
            # First prompt prefill has no earlier context; use the existing dense
            # context kernel for exact causal prompt attention while populating
            # the compressed cache for later decode.
        ...
        if n != 1:
            raise NotImplementedError("Stage-3B2 decode expects one token")
        rabit_out = rabit2_online_decode_attention_triton(
            q,
            kv_cache,
            block_table_row,
            self._rabit2_runtime,
            softmax_scale=self.scale,
        )
        output[:1].copy_(rabit_out)
        return output
