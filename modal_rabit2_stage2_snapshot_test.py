"""RABIT-KV Stage 2 allocator/layout verification using a stable ZIP snapshot.
...
APP_NAME = "rabit2-stage2-allocator-snapshot-test"

SCRIPT_DIR = Path(__file__).resolve().parent
SOURCE_DIR = SCRIPT_DIR / "vllm-kvquant"
SNAPSHOT_PATH = Path(tempfile.gettempdir()) / "rabit2_stage2_vllm_snapshot.zip"
...
image = (
    ...
    .add_local_file(
        str(STABLE_SNAPSHOT),
        "/tmp/rabit2_stage2_vllm_snapshot.zip",
        copy=True,
    )
    .run_commands(
        "rm -rf /root/vllm-kvquant && mkdir -p /root/vllm-kvquant && "
        "python -m zipfile -e /tmp/rabit2_stage2_vllm_snapshot.zip /root/vllm-kvquant"
    )
    .env(
        {
...
def verify_stage2_allocator() -> None:
    ...
    print("=" * 86)
    print("RABIT-2 Stage 2 exact allocator/layout check — stable snapshot")
    print("=" * 86)
