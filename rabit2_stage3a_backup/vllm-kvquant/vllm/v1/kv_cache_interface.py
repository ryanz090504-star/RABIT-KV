...
def rabit2_exact_online_staging_bytes_per_sequence(
    *,
    ...
    """Maximum BF16 staging required to match the quality oracle online.

    K metadata is shared across a 32-token sequence group. Until the current
    group closes, exact streaming updates must retain the original BF16 K
    values for as many as ``group_size - 1`` older tokens; repeatedly
    re-quantizing dequantized K would introduce cumulative drift. The newest
    ``R4`` K/V tokens are also retained in BF16. V needs no additional open
    group because its affine groups run along the head dimension per token.

    This is a per-layer, per-active-sequence staging budget. It is separate
    from the immutable low-bit page pool and does not change page bytes.
    """
    if head_size_v is None:
        head_size_v = head_size_k
    ...
    open_k_tokens = max(sequence_group_size - 1, 0)
    k_tokens = open_k_tokens + residual_tokens
    v_tokens = residual_tokens
    return num_kv_heads * (k_tokens * head_size_k + v_tokens * head_size_v) * 2
