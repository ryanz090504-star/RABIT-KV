...
app = modal.App("rabit-kv-targeted-memory-quality-confirm")
...
def run_benchmark(model_name: str = "LLM-Research/Meta-Llama-3.1-8B-Instruct", mode: str = "final", max_ppl_increase_pct: float = 5.0, screening_ppl_increase_pct: float = 20.0, screen_tokens: int = 128, finalists: int = 16, bit_modes: str = "16,2"):
    import gc
    import math
    import statistics
    import time
    ...
    print("Objective: minimize KV memory subject to a 5% PPL-increase cap.")
    ...
    def candidates_for_bit(bits):
        if bits != 2:
            raise ValueError("Targeted confirmation currently supports bit_modes='16,2'.")
        ...
        # Full boundary search for the user's actual objective:
        # minimize KV memory while keeping PPL increase at or below 5%.
        # Every candidate is full-confirmed because short screening was not
        # reliable near the acceptance boundary.
        add("BASE G32 R4", k_group=32, v_group=32, residual=4)
        add("RES G32 R3", k_group=32, v_group=32, residual=3)

        # Keep V at G32 and sweep the K sequence-group boundary for both R3
        # and R4. Intermediate points are included because K64/R3 passed the
        # 5% cap while K128/R3 failed in the broad search.
        for residual in (3, 4):
            for k_group in (48, 64, 80, 96, 112, 128):
                add(
                    f"GRP K{k_group} V32 R{residual}",
                    k_group=k_group,
                    v_group=32,
                    residual=residual,
                )

        # Directly confirm whether metadata compression can beat the group
        # sweep while preserving the 5% quality cap.
        add(
            "META8g64 G32 R4",
...
def main(
    ...
    screening_ppl_increase_pct: float = 20.0,
