...
class KVQuantMode(IntEnum):
    ...
    INT3_PER_TOKEN_HEAD = 6  # packed 8×int3/3 bytes, dynamic scales
    RABIT_KV2 = 7  # K3/V2 affine payload + compressed metadata; R4 external
    ...
    @property
    def is_rabit_kv2(self) -> bool:
        """True for the asymmetric RABIT-KV K3/V2 cache format."""
        return self == KVQuantMode.RABIT_KV2


def get_kv_quant_mode(kv_cache_dtype: str) -> KVQuantMode:
    """Map a ``kv_cache_dtype`` string to a :class:`KVQuantMode`."""
    if kv_cache_dtype == "rabit_kv2":
        return KVQuantMode.RABIT_KV2
    if kv_cache_dtype == "kvquant_k3":
...
def kvquant_k3_packed_dim(head_size: int) -> int:
    """Packed byte dimension for INT3 values along one attention head."""
    return cdiv(head_size * 3, 8)
...
def rabit2_packed_dim(value_count: int, bits: int) -> int:
    """Packed byte dimension for ``value_count`` low-bit values."""
    if value_count < 0:
        raise ValueError("value_count must be non-negative")
    if bits not in (2, 3):
        raise ValueError("RABIT-KV only supports 2-bit and 3-bit payloads")
    return cdiv(value_count * bits, 8)


def rabit2_encoded_metadata_bytes(
    value_count: int,
    metadata_group_size: int = RABIT2_METADATA_GROUP_SIZE,
) -> int:
    """Bytes used by one compressed metadata tensor.

    Primary metadata values are UINT8 codes. Each group of 64 codes stores one
    BF16 second-level minimum and one BF16 second-level scale (four bytes total).
    """
    if value_count < 0:
        raise ValueError("value_count must be non-negative")
    if metadata_group_size <= 0:
        raise ValueError("metadata_group_size must be positive")
    if value_count == 0:
        return 0
    return value_count + cdiv(value_count, metadata_group_size) * 4


@dataclass(frozen=True)
class Rabit2PageLayout:
    """Opaque byte layout for one quantized RABIT-KV page.

    The page stores old-token K3/V2 payloads and compressed metadata. The
    newest-four-token BF16 residual is sequence state and is intentionally kept
    outside the page pool; reserving it in every page would destroy the target
    compression ratio.
    """
    ...
    head_size_k: int
    head_size_v: int
    k_payload_offset: int
    k_payload_bytes: int
    v_payload_offset: int
    v_payload_bytes: int
    k_min_offset: int
    k_min_bytes: int
    k_scale_offset: int
    ...
    def payload_bytes(self) -> int:
        return self.k_payload_bytes + self.v_payload_bytes
...
def rabit2_page_layout(
    *,
    ...
    """Return the exact page-level K3/V2 payload and metadata layout.

    K uses sequence-axis affine groups of 32 tokens, so the physical block size
    must be a multiple of 32. V uses last-dimension affine groups of 32.
    """
    if block_size <= 0 or block_size % RABIT2_GROUP_SIZE:
        raise ValueError(
            "rabit_kv2 requires --block-size to be a positive multiple of 32"
        )
    if num_kv_heads <= 0 or head_size_k <= 0:
        raise ValueError("num_kv_heads and head sizes must be positive")
    if head_size_v is None:
        head_size_v = head_size_k
    if head_size_v <= 0:
        raise ValueError("head_size_v must be positive")

    k_payload_bytes = (
        block_size
        * num_kv_heads
        * rabit2_packed_dim(head_size_k, RABIT2_K_BITS)
    )
    v_padded_dim = cdiv(head_size_v, RABIT2_GROUP_SIZE) * RABIT2_GROUP_SIZE
    v_payload_bytes = (
        block_size
        * num_kv_heads
        * rabit2_packed_dim(v_padded_dim, RABIT2_V_BITS)
    )

    k_primary_values = (
        (block_size // RABIT2_GROUP_SIZE) * num_kv_heads * head_size_k
    )
    v_primary_values = (
        block_size
        * num_kv_heads
        * cdiv(head_size_v, RABIT2_GROUP_SIZE)
    )
    k_meta_each = rabit2_encoded_metadata_bytes(k_primary_values)
    v_meta_each = rabit2_encoded_metadata_bytes(v_primary_values)

    offset = 0
    k_payload_offset = offset
    offset += k_payload_bytes
    v_payload_offset = offset
    offset += v_payload_bytes
    k_min_offset = offset
    offset += k_meta_each
    k_scale_offset = offset
    offset += k_meta_each
    v_min_offset = offset
    offset += v_meta_each
    v_scale_offset = offset
    offset += v_meta_each

    return Rabit2PageLayout(
        block_size=block_size,
        num_kv_heads=num_kv_heads,
        head_size_k=head_size_k,
        head_size_v=head_size_v,
        k_payload_offset=k_payload_offset,
        k_payload_bytes=k_payload_bytes,
        v_payload_offset=v_payload_offset,
        v_payload_bytes=v_payload_bytes,
        k_min_offset=k_min_offset,
        k_min_bytes=k_meta_each,
        k_scale_offset=k_scale_offset,
        k_scale_bytes=k_meta_each,
        v_min_offset=v_min_offset,
        v_min_bytes=v_meta_each,
        v_scale_offset=v_scale_offset,
        v_scale_bytes=v_meta_each,
        page_bytes=offset,
    )


def rabit2_residual_bytes_per_sequence(
    *,
    num_kv_heads: int,
    head_size_k: int,
    head_size_v: int | None = None,
    ...
    """BF16 bytes for the separate newest-token residual ring."""
    if head_size_v is None:
        head_size_v = head_size_k
    if min(num_kv_heads, head_size_k, head_size_v) <= 0 or residual_tokens < 0:
        raise ValueError("invalid residual dimensions")
    return residual_tokens * num_kv_heads * (head_size_k + head_size_v) * 2


def rabit2_exact_online_staging_bytes_per_sequence(
    *,
    num_kv_heads: int,
    head_size_k: int,
    head_size_v: int | None = None,
    residual_tokens: int = RABIT2_RESIDUAL_TOKENS,
    sequence_group_size: int = RABIT2_GROUP_SIZE,
) -> int:
    """Maximum staging bytes for exact online RABIT-KV2 updates.

    The incomplete 32-token K group retains original BF16 K values so its
    groupwise affine parameters can be recomputed without cumulative drift.
    Matching V values are already reduced to packed INT2 payload, but their
    float32 primary minima/scales remain staged until the page closes; this is
    required to reproduce META8g64 exactly as each metadata group grows.
    The newest ``R4`` K/V tokens remain BF16.

    This is a per-layer, per-active-sequence upper bound, separate from the
    immutable page pool.
    """
    if head_size_v is None:
        head_size_v = head_size_k
    if min(num_kv_heads, head_size_k, head_size_v) <= 0:
        raise ValueError("invalid staging dimensions")
    if residual_tokens < 0 or sequence_group_size <= 0:
        raise ValueError("invalid residual/group size")

    open_tokens = max(sequence_group_size - 1, 0)
    open_k_bf16 = open_tokens * num_kv_heads * head_size_k * 2
    ...
    v_groups_per_head = cdiv(head_size_v, RABIT2_GROUP_SIZE)
    # q_min + q_scale, both float32 primary metadata.
    open_v_primary_metadata = (
        open_tokens * num_kv_heads * v_groups_per_head * 2 * 4
    )
    residual = (
        residual_tokens
        * num_kv_heads
    ...
    return open_k_bf16 + open_v_payload + open_v_primary_metadata + residual


def rabit2_effective_compression_with_staging(
    *,
    context_tokens: int,
    block_size: int,
    num_kv_heads: int,
    ...
    """Allocator-level BF16/RABIT byte ratio including exact online staging."""
    if context_tokens <= 0:
    ...
    page_count = cdiv(context_tokens, block_size)
    staging = rabit2_exact_online_staging_bytes_per_sequence(
        num_kv_heads=num_kv_heads,
        head_size_k=head_size_k,
        head_size_v=head_size_v,
    )
    rabit_bytes = page_count * layout.page_bytes + staging
    bf16_bytes = context_tokens * num_kv_heads * (head_size_k + head_size_v) * 2
    return bf16_bytes / rabit_bytes
...
class AttentionSpec(KVCacheSpec):
    ...
    def page_size_bytes(self) -> int:
        ...
        # RABIT-KV metadata is already part of the opaque page layout.
        if self.kv_quant_mode.is_rabit_kv2:
    ...
    def real_page_size_bytes(self) -> int:
        if self.kv_quant_mode.is_nvfp4:
        ...
        elif self.kv_quant_mode.is_rabit_kv2:
            ...
            head_dim = kvquant_k3_packed_dim(self.head_size)
...
class FullAttentionSpec(AttentionSpec):
    ...
    def real_page_size_bytes(self) -> int:
        if self.kv_quant_mode.is_nvfp4:
            ...
            last_dim = kvquant_k3_packed_dim(self.head_size) + kvquant_k3_packed_dim(
                self.head_size_v
            )
        elif self.kv_quant_mode == KVQuantMode.INT4_PER_TOKEN_HEAD:
...
class MLAAttentionSpec(FullAttentionSpec):
    ...
    def real_page_size_bytes(self) -> int:
        ...
        if self.kv_quant_mode == KVQuantMode.INT3_PER_TOKEN_HEAD:
            head_dim = kvquant_k3_packed_dim(self.head_size)
        elif self.kv_quant_mode == KVQuantMode.INT4_PER_TOKEN_HEAD:
            head_dim = self.head_size // 2
        else:
