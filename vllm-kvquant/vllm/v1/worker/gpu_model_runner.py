...
import functools
# === RABIT2_STAGE3C_GPU_RUNNER_PATCHED ===
import gc
import itertools
import threading
import time
...
class GPUModelRunner(
    ...
    def __init__(
        ...
        if cache_config.cache_dtype == "rabit_kv2":
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> Callable | None:
        ...
        """
        # RABIT-2 sidecars live in the GPU worker process (one bounded state per
        # active request per attention layer). Release them before scheduler
        # blocks can be reused by later requests.
        if (
            self.cache_config.cache_dtype == "rabit_kv2"
            and scheduler_output.finished_req_ids
        ):
            from vllm.v1.attention.ops.rabit_kv2 import rabit2_finish_requests

            rabit2_finish_requests(scheduler_output.finished_req_ids)
        ...
        # Publish final post-condense/post-reorder request identity and the
        # scheduler's CPU-known query/context lengths once per step. Every RABIT
        # attention layer consumes this ordering without per-layer GPU->CPU syncs.
        if self.cache_config.cache_dtype == "rabit_kv2":
            from vllm.v1.attention.ops.rabit_kv2 import rabit2_set_active_batch

            _rabit_req_ids = self.input_batch.req_ids[: self.input_batch.num_reqs]
            _rabit_query_lens = [
                int(scheduler_output.num_scheduled_tokens[req_id])
                for req_id in _rabit_req_ids
            ]
            _rabit_context_lens = [
                int(self.requests[req_id].num_computed_tokens)
                for req_id in _rabit_req_ids
            ]
            rabit2_set_active_batch(
                _rabit_req_ids, _rabit_query_lens, _rabit_context_lens
            )
