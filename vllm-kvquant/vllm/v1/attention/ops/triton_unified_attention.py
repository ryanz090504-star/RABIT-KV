...
def unified_attention(
    ...
    if kv_quant_mode == KVQuantMode.RABIT_KV2:
        raise RuntimeError(
            "RABIT_KV2 uses the backend sidecar dispatch; the generic unified "
            "attention kernel cannot interpret its opaque asymmetric K3/V2 page."
        )

    # Sub-byte packed modes need bespoke kernels (split-dot +
    # sub-byte unpack); everything else goes through the core kernel below.
    if kv_quant_mode == KVQuantMode.INT3_PER_TOKEN_HEAD:
        assert use_causal and not use_per_seq_causal, (
            "INT3_PER_TOKEN_HEAD only supports causal attention"
        )
