"""
Controlled three-way Llama 3.1 8B attention benchmark:

1. BF16 KV cache + automatic backend (expected FlashAttention 3)
2. BF16 KV cache + Triton attention
3. INT3 kvquant_k3 KV cache + Triton attention
...
app = modal.App("vllm-kvquant-controlled-three-way")
...
MODES = {
    "bf16_flash": {
        "label": "BF16 + FlashAttention(auto)",
        "cache_dtype": "auto",
...
def run_controlled_benchmark() -> None:
    modes = ["bf16_flash", "bf16_triton", "int3_triton"]
    data = {}
    logs = {}
    ...
    print("\n" + "=" * 128)
    print("CONTROLLED THREE-WAY BENCHMARK — Llama 3.1 8B on H100")
    print("=" * 128)
    print(
        f"{'Context':>8} | {'BF16 Flash ms/tok':>18} | "
        f"{'BF16 Triton ms/tok':>19} | {'INT3 Triton ms/tok':>19} | "
        f"{'Triton/Flash':>13} | {'INT3/BF16 Triton':>18}"
    )
    print("-" * 128)

    for flash_row, bf16_triton_row, int3_row in zip(
        data["bf16_flash"]["results"],
        data["bf16_triton"]["results"],
        data["int3_triton"]["results"],
    ):
        flash = flash_row["median_ms_per_output_token"]
        bf16_triton = bf16_triton_row["median_ms_per_output_token"]
        int3 = int3_row["median_ms_per_output_token"]

        print(
            f"{flash_row['actual_context_tokens']:>8} | "
            f"{flash:>18.3f} | "
            f"{bf16_triton:>19.3f} | "
            f"{int3:>19.3f} | "
            f"{bf16_triton / flash:>12.3f}x | "
            f"{int3 / bf16_triton:>17.3f}x"
        )

    print("-" * 128)

    flash_capacity = data["bf16_flash"].get("capacity_tokens")
    triton_capacity = data["bf16_triton"].get("capacity_tokens")
    int3_capacity = data["int3_triton"].get("capacity_tokens")

    print(f"BF16 Flash KV capacity:  {flash_capacity} tokens")
    print(f"BF16 Triton KV capacity: {triton_capacity} tokens")
    print(f"INT3 Triton KV capacity: {int3_capacity} tokens")
    if flash_capacity and int3_capacity:
        print(
            f"INT3 capacity improvement over BF16: "
            f"{int3_capacity / flash_capacity:.3f}x"
        )

    print("\nDetected backend log entries:")
    for mode in modes:
        print(f"  {mode}: {backend_patterns[mode]}")

    payload = {
        "results": data,
        "detected_backends": backend_patterns,
    }
    print("\nCONTROLLED_BENCHMARK_JSON=" + json.dumps(payload))
    print("\nCONTROLLED THREE-WAY BENCHMARK COMPLETED")
