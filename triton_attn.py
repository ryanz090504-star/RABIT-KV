...
class TritonAttentionBackend(AttentionBackend):
    ...
    def get_kv_cache_shape(
        ...
        quant_mode = get_kv_quant_mode(cache_dtype_str)
        if quant_mode == KVQuantMode.RABIT_KV2:
            # Opaque raw-byte page. The Stage-3 write/read kernels interpret
            # offsets via ``rabit2_page_layout`` rather than K/V unbind views.
            ...
            return (num_blocks, 1, 1, 1, layout.page_bytes)
        if kv_cache_uses_per_token_head_scales(cache_dtype_str):
            ...
            if quant_mode == KVQuantMode.INT3_PER_TOKEN_HEAD:
                data_head_size = kvquant_k3_packed_dim(head_size)
            elif quant_mode == KVQuantMode.INT4_PER_TOKEN_HEAD:
...
class TritonAttentionImpl(AttentionImpl):
    ...
    def __init__(
        ...
        self._is_rabit_kv2 = self._kv_quant_mode == KVQuantMode.RABIT_KV2
        self._rabit2_runtimes: dict[str, Rabit2SingleSequenceRuntime] = {}
        self._rabit2_logged_multiseq = False
        self._rabit2_logged_chunked = False
        if self._is_rabit_kv2:
            rabit2_register_attention_impl(self)
    ...
    def forward(
        ...
        if self._is_rabit_kv2:
            return self._forward_rabit_kv2(
    ...
    def _forward_rabit_kv2(
        ...
        """Stage-3C multi-request + chunked-prefill RABIT-KV2 path.

        The GPU model runner publishes final request order plus CPU-known
        query/context lengths once per scheduler step. Each request owns an
        independent bounded sidecar. Closed pages are written into the
        scheduler-assigned opaque physical blocks. Initial prefill chunks use
        dense causal attention; later chunks are evaluated token-by-token against
        the compressed prefix plus exact tail. This is correctness-first; Stage 4
        will batch the tail/partial kernels before latency benchmarking.
        """
        if self.attn_type != AttentionType.DECODER:
            raise NotImplementedError("Stage-3C RABIT_KV2 supports decoder attention only")
        if self.alibi_slopes is not None or self.sinks is not None or self.logits_soft_cap:
            raise NotImplementedError(
                "Stage-3C RABIT_KV2 does not yet support ALiBi, sinks, or softcap"
            )
        if self.sliding_window != (-1, -1) or self.chunk_lookback >= 0:
            raise NotImplementedError(
                "Stage-3C RABIT_KV2 does not yet support sliding/chunked attention"
            )
        if attn_metadata.mm_prefix_range_tensor is not None or attn_metadata.rswa_window is not None:
            raise NotImplementedError(
                "Stage-3C RABIT_KV2 does not yet support multimodal prefix or R-SWA"
            )
        ...
        active_batch = rabit2_get_active_batch()
        if active_batch:
            num_seqs = len(active_batch)
            if attn_metadata.query_start_loc.shape[0] != num_seqs + 1:
                raise RuntimeError(
                    "RABIT-2 active request mapping does not match attention batch"
                )
            if sum(item[1] for item in active_batch) != n:
                raise RuntimeError(
                    "RABIT-2 published query lengths do not match num_actual_tokens"
                )
        else:
            # Synthetic profile/warmup forwards happen before scheduler state
            # exists. They are isolated under synthetic IDs and dropped as soon
            # as the first real request batch is published.
            q_starts = attn_metadata.query_start_loc.detach().cpu().tolist()
            seq_lens = attn_metadata.seq_lens.detach().cpu().tolist()
            num_seqs = len(q_starts) - 1
            active_batch = tuple(
                (
                    f"__rabit_profile_{i}",
                    int(q_starts[i + 1] - q_starts[i]),
                    int(seq_lens[i] - (q_starts[i + 1] - q_starts[i])),
                )
                for i in range(num_seqs)
            )

        if num_seqs > 1 and not self._rabit2_logged_multiseq:
            logger.info("RABIT2_STAGE3C_MULTI_SEQUENCE_ACTIVE num_seqs=%d", num_seqs)
            self._rabit2_logged_multiseq = True

        q_cursor = 0
        for seq_idx, (req_id, q_len, context_len) in enumerate(active_batch):
            q_len = int(q_len)
            ...
            if context_len < 0:
                raise RuntimeError(
                    f"RABIT-2 invalid context length for {req_id}: {context_len}"
                )

            block_table_row = attn_metadata.block_table[seq_idx]
            runtime = self._rabit2_runtimes.get(req_id)

            # New request, restarted request, or same-ID abort/resubmit.
            if context_len == 0:
                runtime = Rabit2SingleSequenceRuntime(
                    self.num_kv_heads, self.head_size, self.head_size
                )
                self._rabit2_runtimes[req_id] = runtime
            elif runtime is None:
                raise NotImplementedError(
                    "RABIT-2 received precomputed context without a local sidecar. "
                    "Prefix-cache hits / external KV restore are not supported yet."
                )
            elif runtime.total_tokens != context_len:
                raise RuntimeError(
                    f"RABIT-2 sidecar length mismatch for {req_id}: "
                    f"runtime={runtime.total_tokens}, scheduler_context={context_len}"
                )
            ...
            if context_len == 0 and q_len > 1:
                ...
                context_attention_fwd(
                    ...
                    v=v_seq,
                    o=output[q0:q1],
                    b_start_loc=local_q_start,
                    b_seq_len=local_seq_len,
                    max_input_len=q_len,
                    is_causal=True,
                    softmax_scale=self.scale,
                    sliding_window_q=-1,
            ...
            if context_len > 0 and q_len > 1 and not self._rabit2_logged_chunked:
                logger.info(
                    "RABIT2_STAGE3C_CHUNKED_PREFILL_ACTIVE q_len=%d context_len=%d",
                    q_len,
                    context_len,
                )
                self._rabit2_logged_chunked = True

            # Decode and non-initial chunked prefill share one exact online
    ...
    def do_kv_cache_update(
        ...
        if self._is_rabit_kv2:
            # RABIT_KV2 cache update is deferred to forward(), where the
            # block table / sequence metadata needed by the exact sidecar is available.
            return
        if self._is_per_token_head_quant:
            self._ensure_scale_caches(kv_cache)
            key_cache, value_cache = kv_cache.unbind(1)
    ...
    def fused_rope_kvcache_supported(self):
        if self._is_rabit_kv2 or self._is_per_token_head_quant:
            return False
