...
MANIFEST = OUT_DIR / "final_unified_quality_manifest.json"

EXPECTED_BRANCH = "final/unified-quality"
EXPECTED_RABIT_SHA256 = "108ba8afaac862bc4e070d2dc812493aaeae4e458837c4bda2ac703e72a57524"

RUNS = [
    {
        "name": "01_continuation_ppl",
        "script": "Rabit_KV_continuation_ppl_latest.py",
        "args": [
            "--context-tokens", "1024",
            "--eval-tokens", "128",
            "--samples", "8",
    ...
    {
        "name": "02_multilingual_continuation_ppl",
        "script": "Rabit_KV_multilingual_continuation_ppl.py",
        "args": [
            "--languages", "zh,es",
            "--context-tokens", "1024",
            "--eval-tokens", "128",
    ...
    {
        "name": "03_niah_depth_grid",
        "script": "Rabit_KV_niah_latest.py",
        "args": [
    ...
    {
        "name": "04_passage_retrieval_full10",
        "script": "Rabit_KV_longbench_retrieval_latest.py",
        "args": [
            "--sample-start", "0",
            "--samples", "10",
    ...
    {
        "name": "05_hotpotqa_full20",
        "script": "Rabit_KV_longbench_hotpotqa_latest.py",
        "args": [
            "--sample-start", "0",
            "--samples", "20",
            "--length-bucket", "8k+",
    ...
    {
        "name": "06_qasper_full24",
        "script": "Rabit_KV_longbench_qasper_latest.py",
        "args": [
            "--sample-start", "0",
            "--samples", "24",
            "--length-bucket", "8k+",
...
def preflight() -> dict:
    ...
    if status:
        raise RuntimeError(
            "Working tree must be clean before the final one-shot quality run.\n"
            + status
        )
...
def main() -> None:
    print("RABIT-KV FINAL UNIFIED QUALITY — ONE SHOT")
    print("Policy: K3 / V2 / G32 / R4 / META8g64")
    print("Comparison: BF16 vs frozen final RABIT-KV only")
    print("Rule: measure once; do not tune from these results.")
    print()
    ...
    manifest = {
        "suite": "RABIT-KV final unified quality",
        ...
        "runs": [],
        "note": (
            "This is the single post-performance-freeze final quality rerun. "
            "No parameter tuning is permitted from its results."
        ),
    }
    ...
    for index, spec in enumerate(RUNS, start=1):
        ...
        if code != 0:
            ...
            write_manifest(manifest)
            raise SystemExit(
                f"\nFINAL QUALITY RUN STOPPED at {spec['name']} (exit={code}). "
    ...
    print("\n" + "=" * 118)
    print("FINAL UNIFIED QUALITY RAW RUNS: ALL PASSED")
    print(f"Manifest: {MANIFEST}")
    print(f"Raw logs:  {OUT_DIR}")
    print("Next step: parse these frozen raw logs into final_summary.json; no rerun/tuning.")
    print("=" * 118)
